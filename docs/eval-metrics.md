...
**Not covered (future work):** end-to-end graph quality metrics including graph-level entity resolution (merge/split correctness across the full Neo4j graph), temporal edge integrity after Extraction processing, and query-surface answer quality (the Assistant's graph tools + MCP). These require a running system and are tracked separately.
