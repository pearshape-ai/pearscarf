...
## 1.29.1
- Register the `records` internal expert at startup; `psc expert list` includes it alongside operator-installed experts.

## 1.29.0
- Add the `records` internal expert package — the default MCP record-submission surface bundled with pearscarf.
...
## 1.27.9
- Phase A.2 of OSS-readiness — mypy tightening pass (PEA-145). `mypy pearscarf/` goes from 48 errors across 12 files to 0, wired into both `.pre-commit-config.yaml` (with `types-PyYAML` as an additional dep for the hook) and `.github/workflows/lint.yml` (already had a mypy job from 1.27.8, now actually passes). Config stays loose in `pyproject.toml` (`strict=false`, `ignore_missing_imports=true`, `check_untyped_defs=true`) — per-module tightening is a follow-up, not a blocker. Changes by category: (1) implicit-Optional fixes on 9 function signatures in `mcp/mcp_server.py` — `str = None` → `str | None = None` — including `find_entity`, `get_facts`, `get_connections`, `get_conflicts`, `get_open_commitments`, `get_open_blockers`, `get_recent_activity`. (2) Narrowing after the `_resolve_entity` + `if err: return err` pattern in `mcp_server.py` — added `assert entity is not None` at 8 call sites so mypy sees the non-None branch. (3) Real bug fix: `extraction.py:_debug_write` could dereference `self._debug_dir` when it was `None` — added the `if not self._debug_dir: return` guard that was already present on the paired `_debug_agent` helper. (4) `AssistantAgent._send_tool` declared on the class as `SendMessageTool | None = None` so the dynamic attach in `Assistant._build_agent` is typed. (5) `cli.py:_call_query_tool` — widened `result` to `Any` since it takes different shapes per tool branch. (6) `terminal.py:_sigterm_handler` — frame typed as `FrameType | None` to match `signal.signal`'s expected signature. (7) `repl.py` — renamed the `/history` branch's shadowing `target` to `history_target` with `str | None` annotation. (8) `storage/vectorstore.py:_ensure_collection` — `assert _client is not None` since its only caller (`_get_client`) initializes first. (9) Targeted `# type: ignore` with comments on 4 spots where the narrowing is real but not provable to mypy: `discord_bot.py` channel union (`thread.send` — always a TextChannel/Thread in our flow), `tracing.py` langsmith `traceable` overload (run_type accepts our free-form str at runtime even though its type is Literal), `install.py` guard against a None-returning `entry.get("name")` in a set-comprehension. No runtime behavior change; er-foundations eval clean on Haiku after the pass (11/11 merge recall, 3/3 entity merge rate, 0 false merges, 25k/6k tokens — within baseline).

## 1.27.8
- Phase A of OSS-readiness prep (PEA-144). Adds `ruff` as the linter + formatter (replaces the `black`/`flake8`/`isort`/`pyupgrade` stack with one tool) and `pre-commit` to enforce it at commit time. Ruff config sits in `pyproject.toml` under `[tool.ruff]` — line length 100, `target-version = "py312"`, rule set `E,F,W,I,UP,B` (pycodestyle errors + pyflakes + whitespace + isort + pyupgrade + bugbear). `E501` ignored (formatter owns wrapping; some Cypher / prompt lines are legitimately long) and `B008` ignored (common Click / dataclass default-arg pattern). Per-file ignores for `scripts/*` and `experts/*/*_ingest.py` to allow the env-setup-before-import pattern both use. Dev dependency declared in new `[dependency-groups] dev`. `.pre-commit-config.yaml` ships `ruff check --fix` + `ruff format` hooks; `.github/workflows/lint.yml` runs the same two checks on every PR and push to main. One-time `ruff format .` + `ruff check --fix .` pass applied as part of this commit — 104 auto-fixes (import sort, unused imports, syntax upgrades) across 41 files, plus 14 manual fixes for the 20 issues ruff couldn't auto-fix: rename 5 uses of `l` as a loop variable to `lbl`, replace 3 bare `raise` inside `except` with `raise … from err`, add `strict=False` to 3 `zip()` calls, drop 2 unused locals (`stats`, `result`), and `# noqa` on 1 optional-hook `Consumer._setup` + 6 intentional late imports (cli.py registers command groups after `cli` is declared; store.py lazy-loads `hashlib`/`secrets` next to the `# --- MCP Keys ---` boundary). Mypy intentionally deferred to a Phase A.2 issue — `mypy pearscarf/` surfaces 48 errors across 12 files in its first pass, more than the 2-hour Phase A scope can absorb. No runtime behavior change; er-foundations eval clean before and after (11/11 merge recall, 3/3 entity merge, 0 false merges).
- Fix eval runs against the shipped-default onboarding. `pearscarf/eval/runner.py:run_er_eval` now calls `os.environ.pop("ONBOARDING_PROMPT_PATH", None)` as its first step, so a deployment's operator-specific onboarding (which legitimately lists shared fixture names from `pearscarf-eval` as "never real") doesn't opinionate scoring against those same fixture datasets. Without this, running `psc eval er` under a live operator config zeroes the eval by filtering out the exact entities being scored — which is what surfaced this — even though the stub default works fine for a blank-slate clone. Uses the existing `ONBOARDING_PROMPT_PATH` env-var mechanism rather than adding new API. Verified on er-foundations: 100% on every metric, 28,823 input / 6,211 output tokens on Haiku 4.5 (matches the pre-regression baseline).

## 1.27.7
- Tighten linearscarf extraction guidance and keep it deployment-agnostic. `experts/linearscarf/knowledge/extraction.md` rewritten to enforce a single absolute rule ("the Project entity for a Linear issue is exactly the value on the `Project:` metadata line; nothing else is a project") and to enumerate — in generic, non-deployment-specific terms — the kinds of things that a Linear issue body looks like a project but isn't: the issue identifier, the issue title, the URL slug, code identifiers (class/module/tool names), abbreviations, work themes from body headings, other issue identifiers linked in the body. Also adds an "illustrative examples and hypotheticals are NOT facts" section for records that contain `Example: …` passages as teaching material. No URLs, issue IDs, or vocabulary from any specific deployment are in the file — it reads the same for any Linear instance. Verified across 42 real Linear records: with Haiku 4.5 + the new deployment-specific onboarding described below, Project extraction is now limited to the two real projects (vs. 13+ spurious project nodes on the pre-change dogfood run with the old prompt on gpt-4.1-mini).
- Revert `pearscarf/knowledge/onboarding.md` to a neutral public stub. The stub explains that the shipped content is intentionally minimal — operators are expected to set `ONBOARDING_PROMPT_PATH` to a local file that names their world (who the operator is, what the real projects/companies/people are, what fixture/test-dataset names must never be extracted as entities). The earlier write-up of a specific deployment's onboarding (with pearshape-ai project names, eval-fixture names, code vocabulary) moves to `infrascarf/runtime/onboarding.md` where it belongs. This keeps the public `pearscarf` repo deployment-agnostic; operator-specific context lives in the private deploy repo.

## 1.27.6
- Multi-provider LLM support. `BaseAgent` stops hardcoding `anthropic.Anthropic` and calls through a new `LLMClient` adapter in `pearscarf/agents/llm_client.py`. Concrete `AnthropicClient` + `OpenAIClient`, normalized `LLMResponse` / `LLMToolCall` / `LLMUsage` dataclasses, a `get_llm_client(model)` factory, and a `resolve_provider(model, explicit)` helper that infers the provider from the model name (`claude-*` → anthropic, `gpt-*` / `o1-*` / `o3-*` / `o4-*` → openai) or accepts an explicit `PROVIDER` env var override. Unknown-model names error cleanly with the list of supported prefixes rather than silently picking a default. `BaseAgent.run` now reads the normalized `response.tool_calls` / `response.stop_reason`; tool-result messages and assistant-message round-tripping go through `client.format_tool_results(...)` and `client.build_assistant_message(...)` so Anthropic's content-block shape vs OpenAI's `role: "tool"` shape is handled inside each client, invisibly to the agent loop. `pearscarf/tracked_call.py`'s `tracked_anthropic_call` is renamed to `tracked_call`, takes any `LLMClient`, reads the normalized `LLMResponse.usage` (works for both providers' usage fields), and populates `llm_calls.provider` from `client.provider_name` instead of hardcoding `"anthropic"`. `pearscarf/config.py` exposes `OPENAI_API_KEY` and `PROVIDER`. `pyproject.toml` adds the `openai` dependency. `.env.example` reshuffled to put LLM provider + model up top.
- `OpenAIClient.invoke` handles two OpenAI-specific quirks: (1) `max_tokens` vs `max_completion_tokens` — `gpt-5*` and `o-series` models reject `max_tokens` with a 400, so the client picks the right kwarg based on the model-name prefix; (2) OpenAI's `prompt_tokens` usage field includes cached tokens, so the client subtracts `prompt_tokens_details.cached_tokens` to report a "fresh" `input_tokens` count consistent with Anthropic's semantics — dashboards and `scripts/cost.py` don't need to know which provider produced a row. OpenAI's automatic prompt caching (no markers, ≥1024-token prefix) populates `cache_read_tokens` on its own; confirmed 26k–35k cached reads per er-foundations run on `gpt-4.1-mini`.
- `scripts/cost.py` PRICES table extended with six OpenAI models at public rates as of 2026-04-23: `gpt-4o` ($2.50/$10), `gpt-4.1-mini` ($0.40/$1.60), `gpt-4o-mini` ($0.15/$0.60), `gpt-5` ($0.625/$5.00), `gpt-5-mini` ($0.25/$2.00), `gpt-5-nano` ($0.05/$0.40). Each carries a cached-read rate; `cache_write` is set equal to input since OpenAI doesn't bill a separate cache-write rate.
- Verification on er-foundations. **Anthropic no-regression:** Haiku 4.5 via the new adapter layer landed at $0.0793, 12 calls, 100% on all quality metrics — matches the 1.27.5 baseline. **OpenAI smoke:** `gpt-4o-mini` alternates between perfect (100% quality, $0.0088) and noisy (11 nodes vs 6 expected, duplicates) run-to-run — cheap but unreliable. `gpt-5-mini` failed quality (7/11 merge recall, 0/3 entity merge) and inflates cost via reasoning tokens ($0.034/run with 14k output tokens vs 6k for other models). **`gpt-4.1-mini` is the winner:** 2/2 clean runs at 100% on every metric, mean cost $0.019/run — 4× cheaper than Haiku at identical quality, 2× the price of `gpt-4o-mini` but without the duplication failure mode. Recommended deployment default for extraction.

## 1.27.5
- Consolidate the Extraction agent's four-tool graph-read cascade (`find_entity` → `search_entities` → `check_alias` → `get_entity_context`) into a single `resolve_entity(entity_type, name, [identifier])` tool. The new tool runs all three lookups internally — identifier match first (email for persons, domain for companies), then exact-name match, then alias self-loop, then fuzzy fallback — and attaches brief context (top 3 facts + top 3 connections) to every returned entity automatically. Returns one of: `match: "definitive"` (entity + context), `match: "candidates"` (list with contexts, for agent disambiguation), or `match: "none"`. Exact-name collisions and alias collisions both return candidates rather than an arbitrary `LIMIT 1` pick — fixes a latent correctness gap where two `James Whitfield` nodes could have the wrong one returned confidently. Implemented in `pearscarf/graph_access_tools.py`; `pearscarf/extraction.py:_run_extractor_agent` now registers only `ResolveEntityTool` + `SaveExtractionTool` on the Extractor; the old four tools remain in the module and stay available to other consumers (Triage). `pearscarf/knowledge/extractor/extractor_agent.md` rewritten to describe the single-call workflow with explicit guidance that the returned context is the one chance to look at the entity's neighborhood — don't re-query.
- Verified on er-foundations (Haiku + cache): 17 → 12 calls (−29%), per-record turn count flat at 3 (was 4–5), cost $0.0799 → $0.0747 (noise range), quality unchanged at 11/11 merge recall, 3/3 entity merge rate, 0 false merges. Verified on base-scenario (36 records, Haiku + cache): 161 → 105 calls (−35%), 1,107,658 → 160,302 fresh input tokens (−86% due to 516,938 cache reads keeping the stable prefix warm across all records), cost $1.3840 → $0.4452 (−68%). Cumulative saving across the day (uncached cascade → cached + resolve_entity) is −68% on base-scenario workloads.
- Graph-quality tradeoff on base-scenario: project-node deduplication regressed. Prior run had one missed merge (`Meridian Integration` not merged into seeded `Meridian API Integration`); this run has three (`Meridian Integration`, `Meridian contract`, `Meridian partnership` all created as new nodes when they likely belong with `Meridian Deal` / `Meridian API Integration`). Root cause is a deliberate "prefer new over a wrong merge" nudge in the rewritten prompt, which biases the agent toward creating new nodes when fuzzy candidates are ambiguous. People and companies are unchanged (8/8 seeded people + 1 new, 4/4 seeded companies + 1 new). Accepted: curation runs downstream to dedupe project nodes, and false merges are much harder to reverse than duplicates.

## 1.27.4
- Enable Anthropic prompt caching on every `BaseAgent` call. `pearscarf/agents/base.py` now wraps the `system` kwarg in a content block carrying `cache_control: {"type": "ephemeral"}` and marks the last tool schema with `cache_control` too, so the stable per-agent prefix (system prompt + tool definitions) is served from cache after the first call inside each 5-min window. Per-record user messages and accumulated tool results stay outside the cache — correctly — since they change every call. Verified end-to-end on the Haiku er-foundations eval: fresh `input_tokens` 95,674 → 23,456 (−75%), `cache_creation_tokens` 0 → 15,961, `cache_read_tokens` 0 → 61,778; cost via `scripts/cost.py --latest-eval` 12.9¢ → 8.0¢ (−38%) with quality unchanged (11/11 merge recall, 3/3 entity merge rate, 0 false merges). `llm_calls.cache_creation_tokens` and `cache_read_tokens`, which had been reading 0 since 1.26.11 shipped, now populate as expected. Extrapolated savings on base-scenario ($1.38 → ~$0.85) will be confirmed next time we run it.
- Fix a latent bug in `pearscarf/tracked_call.py`: `tracked_anthropic_call` called `.encode("utf-8")` directly on `kwargs["system"]` assuming it was always a string. Anthropic accepts `system` as either a string or a list of content blocks, and the caching change above needs the list form. Added a `_stringify_system()` helper that flattens either shape for hashing and `llm_prompts.body` storage. Without this, the first cached run raised `AttributeError` before the API call, silently ate the turn (records marked `indexed=true` but no `llm_calls` rows), and eval reported 0 nodes — caught immediately in the verification run.
...
## 1.26.11
- Observability substrate for LLM calls. Three new Postgres tables (`runtimes`, `llm_prompts`, `llm_calls`) and a new `pearscarf/tracked_call.py` module that wraps `anthropic.messages.create` and logs one row per LLM turn. `runtimes` captures a version snapshot (pearscarf version + installed expert versions + hostname/pid) once per Consumer boot so any historical call can be reverse-engineered. `llm_prompts` holds deduped composed system prompt bodies via sha256 hash; `tracked_call` upserts on first sighting (`ON CONFLICT DO NOTHING`). `llm_calls` carries per-turn facts — run_id/turn_index grouping, provider (hardcoded `anthropic` for now; schema is provider-agnostic), model, prompt_hash FK, stop_reason, tool names used, input/output/cache tokens, latency, record_id/session_id context ties, error column for failures. No cost/dollar fields — tokens are ground-truth; pricing computed externally at read time.
- Wiring: `Consumer._loop()` calls `register_runtime(self.name)` at thread startup, storing `runtime_id` and setting the `_runtime_id_var` / `_consumer_var` ContextVars. Each subclass `_handle` sets `_record_id_var` (record-side consumers) or `_session_id_var` (bus-side SessionConsumer). `BaseAgent.run()` manages `_run_id_var` (fresh uuid per run) + `_turn_index_var` (incremented per turn) and routes the LLM call through `tracked_anthropic_call` instead of `self._client.messages.create`. Observability failures (DB unreachable, log row rejection) are swallowed and logged at warning — the main flow never breaks on tracking.
- See `notion/design/observability-and-safety.md` for the design; `lab/design/2026-04-22-observability-phase-4.md` for the implementation-level details and deferred safety-rail items (bus access control, daily token budgets, cost-cap UX) still to be thought through.
