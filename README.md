...
Emails read by the Gmail expert are persisted to a System of Record (`records` + `emails` tables) with deduplication. A background Indexer processes new records through an LLM to extract entities, relationships, and facts into a knowledge graph.
